...
// File: rg_etc1.cpp - Fast, high quality ETC1 block packer/unpacker - Rich Geldreich <richgel99@gmail.com> v1.04 
...
namespace rg_etc1
{
   ...
   template<typename T> finline T minimum(T a, T b) { return (a < b) ? a : b; }
   template<typename T> finline T minimum(T a, T b, T c) { return minimum(minimum(a, b), c); }
   template<typename T> finline T maximum(T a, T b) { return (a > b) ? a : b; }
   template<typename T> finline T maximum(T a, T b, T c) { return maximum(maximum(a, b), c); }
   template<typename T> finline T clamp(T value, T low, T high) { return (value < low) ? low : ((value > high) ? high : value); }
   template<typename T> finline T square(T value) { return value * value; }
   template<typename T> finline void zero_object(T& obj) { memset((void*)&obj, 0, sizeof(obj)); }
   template<typename T> finline void zero_this(T* pObj) { memset((void*)pObj, 0, sizeof(*pObj)); }
...
#define RG_ETC1_ARRAY_SIZE(X) (sizeof(X) / sizeof(decay_array_to_subtype(X)))
   ...
   {
      static finline int clamp(int v) { if (v & 0xFFFFFF00U) v = (~(static_cast<int>(v) >> 31)) & 0xFF; return v; }
   ...
   public:
      ...
      finline color_quad_u8()
      {
      }

      finline color_quad_u8(const color_quad_u8& other) : m_u32(other.m_u32)
      {
      }

      explicit finline color_quad_u8(parameter_t y, parameter_t alpha = component_traits::cMax)
      {
         set(y, alpha);
      }

      finline color_quad_u8(parameter_t red, parameter_t green, parameter_t blue, parameter_t alpha = component_traits::cMax)
      {
         set(red, green, blue, alpha);
      }

      explicit finline color_quad_u8(eNoClamp, parameter_t y, parameter_t alpha = component_traits::cMax)
      {
         set_noclamp_y_alpha(y, alpha);
      }

      finline color_quad_u8(eNoClamp, parameter_t red, parameter_t green, parameter_t blue, parameter_t alpha = component_traits::cMax)
      {
      ...
      finline color_quad_u8& operator= (const color_quad_u8& other)
      {
         m_u32 = other.m_u32;
         return *this;
      }

      finline color_quad_u8& set_rgb(const color_quad_u8& other)
      {
         r = other.r;
         g = other.g;
         b = other.b;
      ...
      finline color_quad_u8& operator= (parameter_t y)
      {
         set(y, component_traits::cMax);
         return *this;
      }

      finline color_quad_u8& set(parameter_t y, parameter_t alpha = component_traits::cMax)
      {
         y = clamp(y);
         alpha = clamp(alpha);
      ...
      finline color_quad_u8& set_noclamp_y_alpha(parameter_t y, parameter_t alpha = component_traits::cMax)
      {
      ...
      finline color_quad_u8& set(parameter_t red, parameter_t green, parameter_t blue, parameter_t alpha = component_traits::cMax)
      {
      ...
      finline color_quad_u8& set_noclamp_rgba(parameter_t red, parameter_t green, parameter_t blue, parameter_t alpha)
      {
      ...
      finline color_quad_u8& set_noclamp_rgb(parameter_t red, parameter_t green, parameter_t blue)
      {
         RG_ETC1_ASSERT( (red >= component_traits::cMin) && (red <= component_traits::cMax) );
         RG_ETC1_ASSERT( (green >= component_traits::cMin) && (green <= component_traits::cMax) );
         RG_ETC1_ASSERT( (blue >= component_traits::cMin) && (blue <= component_traits::cMax) );
      ...
      static finline parameter_t get_min_comp() { return component_traits::cMin; }
      static finline parameter_t get_max_comp() { return component_traits::cMax; }
      static finline bool get_comps_are_signed() { return component_traits::cSigned; }

      finline component_t operator[] (uint i) const { RG_ETC1_ASSERT(i < cNumComps); return c[i]; }
      finline component_t& operator[] (uint i) { RG_ETC1_ASSERT(i < cNumComps); return c[i]; }

      finline color_quad_u8& set_component(uint i, parameter_t f)
      {
      ...
      finline color_quad_u8& set_grayscale(parameter_t l)
      {
      ...
      finline color_quad_u8& clamp(const color_quad_u8& l, const color_quad_u8& h)
      {
      ...
      finline color_quad_u8& clamp(parameter_t l, parameter_t h)
      {
         for (uint i = 0; i < cNumComps; i++)
   ...
   struct vec3F
   {
      ...
      finline vec3F() { }
      finline vec3F(float s) { m_s[0] = s; m_s[1] = s; m_s[2] = s; }
      finline vec3F(float x, float y, float z) { m_s[0] = x; m_s[1] = y; m_s[2] = z; }
      
      finline float operator[] (uint i) const { RG_ETC1_ASSERT(i < 3); return m_s[i]; }

      finline vec3F& operator += (const vec3F& other) { for (uint i = 0; i < 3; i++) m_s[i] += other.m_s[i]; return *this; }

      finline vec3F& operator *= (float s) { for (uint i = 0; i < 3; i++) m_s[i] *= s; return *this; }
   };
   ...
   struct etc1_block
   {
      ...
      finline uint get_byte_bits(uint ofs, uint num) const
      {
      ...
      finline void set_byte_bits(uint ofs, uint num, uint bits)
      {
         RG_ETC1_ASSERT((ofs + num) <= 64U);
      ...
      finline bool get_flip_bit() const 
      {
         return (m_bytes[3] & 1) != 0;
      }   

      finline void set_flip_bit(bool flip)
      {
         m_bytes[3] &= ~1;
         m_bytes[3] |= static_cast<uint8>(flip);
      }

      finline bool get_diff_bit() const
      {
         return (m_bytes[3] & 2) != 0;
      }

      finline void set_diff_bit(bool diff)
      {
         m_bytes[3] &= ~2;
      ...
      finline uint get_inten_table(uint subblock_id) const
      {
      ...
      finline void set_inten_table(uint subblock_id, uint t)
      {
         RG_ETC1_ASSERT(subblock_id < 2);
         RG_ETC1_ASSERT(t < 8);
         const uint ofs = subblock_id ? 2 : 5;
      ...
      finline uint get_selector(uint x, uint y) const
      {
      ...
      finline void set_selector(uint x, uint y, uint val)
      {
      ...
      finline void set_base4_color(uint idx, uint16 c)
      {
         if (idx)
         {
      ...
      finline uint16 get_base4_color(uint idx) const
      {
         uint r, g, b;
         if (idx)
         {
      ...
      finline void set_base5_color(uint16 c)
      {
      ...
      finline uint16 get_base5_color() const
      {
      ...
      finline uint16 get_delta3_color() const
      {
      ...
      static finline void unscaled_to_scaled_color(color_quad_u8& dst, const color_quad_u8& src, bool color4)
      {
         if (color4)
         {
            dst.r = src.r | (src.r << 4);
...
#undef RG_ETC1_GET_KEY_FROM_INDEX
   ...
   {
      finline etc1_solution_coordinates() :
      ...
      finline etc1_solution_coordinates(uint r, uint g, uint b, uint inten_table, bool color4) : 
      ...
      finline etc1_solution_coordinates(const color_quad_u8& c, uint inten_table, bool color4) : 
      m_unscaled_color(c),
         m_inten_table(inten_table),
         m_color4(color4)
      {
      ...
      finline etc1_solution_coordinates(const etc1_solution_coordinates& other)
      {
         *this = other;
      }

      finline etc1_solution_coordinates& operator= (const etc1_solution_coordinates& rhs)
      {
      ...
      finline color_quad_u8 get_scaled_color() const
      {
         int br, bg, bb;
         if (m_color4)
         {
      ...
      finline void get_block_colors(color_quad_u8* pBlock_colors)
      {
         int br, bg, bb;
         if (m_color4)
         {
   ...
   public:
      ...
      struct results
      {
         ...
         finline results& operator= (const results& rhs)
         {
   ...
   static finline int mul_8bit(int a, int b) { int t = a*b + 128; return (t + (t >> 8)) >> 8; }
